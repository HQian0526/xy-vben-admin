...
<template>
          ...
          <ElCol :span="item.span ? item.span : 12">
            <ElFormItem :label="item.label" :prop="item.name">
              ...
              <el-input-number v-if="item.type === 'number'" v-model="formData[item.name]" :min="item.min ? item.min : 0"
                :max="item.max ? item.max : 999999" :readonly="item.readonly ? item.readonly : false" />
  ...
  <!-- 选择用户弹窗 -->
  <SelectPeople :key="selectPeopleKey" :visible="selectPeopleVisible" :selected-users="selectedUsers" :select-more="false"
    @close="closeUserDialog" @confirm="confirmUserDialog" />
</template>
